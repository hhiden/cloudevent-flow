...
/**
 * Watches for new deployments of a dataflow
 * @author hhiden
 */
public class DeploymentWatcher implements Watcher<ConfigMap>, Runnable {
    ...
    public DeploymentWatcher(DeploymentController controller, String cmPredicate) {
    ...
    public void eventReceived(Watcher.Action action, ConfigMap configMap) {
        ...
        if (labelValid(cmPredicate, configMap)) {
            ...
            try {
                switch (action) {
                    case ADDED:
                        logger.info("Added: " + configMap.getMetadata().getName());
                        //controller.onAdded(configMap);
                        break;
                    case MODIFIED:
                        logger.info("Modified: " + configMap.getMetadata().getName());
                        //controller.onModified(configMap);
                        break;
                    case DELETED:
                        logger.info("Deleted: " + configMap.getMetadata().getName());
